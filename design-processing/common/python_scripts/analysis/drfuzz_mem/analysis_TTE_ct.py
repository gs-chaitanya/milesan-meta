...
import glob
import pandas as pd
TAINT_MISMATCH_PATH = "/mnt/milesan-data-ccs/CT-VIOLATIONS-TTE/"
FIGSIZE_FLAT = (8,2)
LABELSIZE = 15
TICKSIZE = 12
LEGENDSIZE = 12
...
for i,file in enumerate(glob.glob(TAINT_MISMATCH_PATH + "**/reduce.log", recursive=True)):
    with open(file, "r") as f:
        p = json.load(f)
...
plt.savefig(TTE_PLOTS_PATH + "/tte_ct_violations_cva6.svg")
...
ax.grid()
ax.set_xticklabels(["div","divu","divw","divuw","rem","remu","remw","remuw"], size=TICKSIZE)
plt.savefig(TTE_PLOTS_PATH + "/tte_ct_violations_openc910.svg")
