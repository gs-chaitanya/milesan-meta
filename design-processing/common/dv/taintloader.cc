...
std::vector<std::unordered_map<size_t, std::pair<std::vector<uint8_t>,std::vector<uint8_t>> >> word_taint_data_contents;
static size_t glob_word_width_bytes = -1;

extern "C" {
  void init_taint_vectors(long long num_taints);
  void init_taint_data_vectors(long long num_taints);
  ...
  void read_taints_and_data(const char* filename, int word_width_bytes);
  char get_next_taint_word(long long taint_id, long int *word_address, const svOpenArrayHandle svbuffer);
  char get_next_taint_and_data_word(long long taint_id, long int *word_address, const svOpenArrayHandle sv_tbuffer,  const svOpenArrayHandle sv_dbuffer);
}
...
extern "C" void init_taint_data_vectors(long long num_taints) {
  word_taint_data_contents.resize(num_taints);
  // for(auto &taint_data_pair: word_taint_data_contents) taint_data_pair.clear();
}
...
// @return 1 if this is a valid response, 0 if there is nothing anymore to supply, and the outputs should be ignored.
extern "C" char get_next_taint_and_data_word(long long taint_id, long int *word_address, const svOpenArrayHandle sv_tbuffer,  const svOpenArrayHandle sv_dbuffer) {
  static auto itr = word_taint_data_contents[taint_id].begin();

  // If the end of the structure was reached.
  if (itr == word_taint_data_contents[taint_id].end()) {
    // Set it back to the beginning so another mem can read it as well (used in Ibex).
    itr = word_taint_data_contents[taint_id].begin();
    return 0;
  }
  ...
  *word_address = itr->first;
  ...
  // Fill the svbuffer with the word contents
  unsigned char* tbuf = (unsigned char*)svGetArrayPtr(sv_tbuffer);
  unsigned char* dbuf = (unsigned char*)svGetArrayPtr(sv_dbuffer);
  for (size_t byte_id_in_word = 0; byte_id_in_word < glob_word_width_bytes; byte_id_in_word++){
    // printf("  Byte %d: %lx\n", byte_id_in_word, itr->second[byte_id_in_word]);
    tbuf[byte_id_in_word] = itr->second.first[byte_id_in_word];
    dbuf[byte_id_in_word] = itr->second.second[byte_id_in_word];
  }
...
extern "C" void read_taints_and_data(const char* filename, int word_width_bytes) {
  assert (word_width_bytes > 0);
  glob_word_width_bytes = word_width_bytes;
  ...
  std::string num_bytes_str;
  std::string byte_taint_contents_str;
  std::string byte_data_contents_str;
  ...
  while (std::getline(f, tmp_line)) {
    ...
    isstream >> num_bytes_str;
    isstream >> byte_taint_contents_str;
    isstream >> byte_data_contents_str;
    ...
    for (size_t curr_byte_addr = base_addr; curr_byte_addr < base_addr+num_bytes; curr_byte_addr ++) {
      ...
      // Compute the curr byte content
      size_t byte_offset = curr_byte_addr - base_addr;
      uint8_t tbyte_val = (char_to_hex(byte_taint_contents_str[2*byte_offset]) << 4) | char_to_hex(byte_taint_contents_str[2*byte_offset+1]); 
      uint8_t dbyte_val = (char_to_hex(byte_data_contents_str[2*byte_offset]) << 4) | char_to_hex(byte_data_contents_str[2*byte_offset+1]); 

      // If the word is not yet in the new words struct, then add it.
      if (word_taint_data_contents[taint_id].find(curr_abs_word_addr) == word_taint_data_contents[taint_id].end()) {
        std::vector<uint8_t> new_taint_content(word_width_bytes, 0);
        std::vector<uint8_t> new_data_content(word_width_bytes, 0);
        std::pair<std::vector<uint8_t>,std::vector<uint8_t>> taint_data_pair(new_taint_content, new_data_content);
        word_taint_data_contents[taint_id].insert({curr_abs_word_addr, taint_data_pair});
      }

      // Add the content to the word
      size_t byte_id_in_word = curr_byte_addr % word_width_bytes;
      word_taint_data_contents[taint_id][curr_abs_word_addr].first[byte_id_in_word] = tbyte_val;
      word_taint_data_contents[taint_id][curr_abs_word_addr].second[byte_id_in_word] = dbyte_val;
    }
  }
