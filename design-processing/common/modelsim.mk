...
build_drfuzz_mem_trace_modelsim:         $(MODELSIM_PATH_TO_BUILD_TCL) $(MODELSIM_SV_DRFUZZ_MEM)     | $(MODELSIM_WORKDIR) modelsim traces logs
	cd $(MODELSIM_WORKDIR); CASCADE_DIR=$(CASCADE_DIR) TRACE=trace     INSTRUMENTATION=drfuzz_mem CASCADE_META_COMMON=$(CASCADE_DESIGN_PROCESSING_ROOT)/common     MODELSIM_INCDIRSTR=$(MODELSIM_INCDIRSTR) MODELSIM_VLOG_COVERFLAG=$(MODELSIM_VLOG_COVERFLAG) TOP_SOC=$(TOP_SOC) VARIANT_ID=$(VARIANT_ID) SV_TOP=src/$(TOP_SOC)_drfuzz_mem.sv        SV_MEM=$(CASCADE_DESIGN_PROCESSING_ROOT)/common/src/ift_sram_mem.sv SV_TB=dv/sv/tb_top_taints.sv CURR_OPENTITAN_ROOT=$(CURR_OPENTITAN_ROOT) $(MODELSIM_VERSION) vsim -64 -c -do $<
...
build_drfuzz_mem_notrace_modelsim:         $(MODELSIM_PATH_TO_BUILD_TCL) $(MODELSIM_SV_DRFUZZ_MEM)     | $(MODELSIM_WORKDIR) modelsim traces logs
	cd $(MODELSIM_WORKDIR); CASCADE_DIR=$(CASCADE_DIR) TRACE=notrace     INSTRUMENTATION=drfuzz_mem CASCADE_META_COMMON=$(CASCADE_DESIGN_PROCESSING_ROOT)/common     MODELSIM_INCDIRSTR=$(MODELSIM_INCDIRSTR) MODELSIM_VLOG_COVERFLAG=$(MODELSIM_VLOG_COVERFLAG) TOP_SOC=$(TOP_SOC) VARIANT_ID=$(VARIANT_ID) SV_TOP=src/$(TOP_SOC)_drfuzz_mem.sv        SV_MEM=$(CASCADE_DESIGN_PROCESSING_ROOT)/common/src/ift_sram_mem.sv SV_TB=dv/sv/tb_top_taints.sv CURR_OPENTITAN_ROOT=$(CURR_OPENTITAN_ROOT) $(MODELSIM_VERSION) vsim -64 -c -do $<
	touch modelsim/drfuzz_mem.log
...
$(RERUN_MODELSIM_TARGETS_NOTRACE):   rerun_%_notrace_modelsim:   $(CASCADE_DESIGN_PROCESSING_ROOT)/common/modelsim/modelsim_run.tcl | $(MODELSIM_WORKDIR) modelsim traces logs
	cd $(MODELSIM_WORKDIR); TOP_SOC=$(TOP_SOC) CASCADE_DIR=$(CASCADE_DIR) VARIANT_ID=$(VARIANT_ID) TRACE=notrace   INSTRUMENTATION=$* TRACEFILE=$(TRACEFILE)                   MODELSIM_VSIM_COVERFLAG=$(MODELSIM_VSIM_COVERFLAG) MODELSIM_VSIM_COVERPATH=$(MODELSIM_VSIM_COVERPATH) $(MODELSIM_VERSION) timeout $(MODELSIM_TIMEOUT) vsim -64 -c -do $<
$(RERUN_MODELSIM_TARGETS_TRACE):     rerun_%_trace_modelsim:     $(CASCADE_DESIGN_PROCESSING_ROOT)/common/modelsim/modelsim_run.tcl | $(MODELSIM_WORKDIR) modelsim traces logs
	cd $(MODELSIM_WORKDIR); TOP_SOC=$(TOP_SOC) CASCADE_DIR=$(CASCADE_DIR) VARIANT_ID=$(VARIANT_ID) TRACE=trace     INSTRUMENTATION=$* TRACEFILE=$(TRACEFILE)                   MODELSIM_VSIM_COVERFLAG=$(MODELSIM_VSIM_COVERFLAG) MODELSIM_VSIM_COVERPATH=$(MODELSIM_VSIM_COVERPATH) $(MODELSIM_VERSION) timeout $(MODELSIM_TIMEOUT_TRACE_EN) vsim -64 -c -do $<
$(RERUN_MODELSIM_TARGETS_TRACE_FST): rerun_%_trace_fst_modelsim: $(CASCADE_DESIGN_PROCESSING_ROOT)/common/modelsim/modelsim_run.tcl | $(MODELSIM_WORKDIR) modelsim traces logs
	cd $(MODELSIM_WORKDIR); TOP_SOC=$(TOP_SOC) CASCADE_DIR=$(CASCADE_DIR) VARIANT_ID=$(VARIANT_ID) TRACE=trace_fst INSTRUMENTATION=$* TRACEFILE=$(TRACEFILE) MODELSIM_NOQUIT=1 MODELSIM_VSIM_COVERFLAG=$(MODELSIM_VSIM_COVERFLAG) MODELSIM_VSIM_COVERPATH=$(MODELSIM_VSIM_COVERPATH) timeout $(MODELSIM_TIMEOUT_TRACE_EN) vsim -64 -do $<
