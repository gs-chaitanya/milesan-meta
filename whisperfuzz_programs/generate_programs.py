...
def generate_program(inst_str: str, imm: int, stopsig_addr: int, regdump_addr: int, clear_csr = bool):
    ...
    if clear_csr:
        prog_str += "csrwi mcycle, 0\n"
    prog_str += f"{inst_str} a3, a4\n" if "mv" in inst_str or "c" in inst_str else  f"{inst_str} a3, a3, a4\n"
...
for dut in DUTS:
    ...
    for instr_str in INSTR_STRS+CINSTR_STR:
        for imm in IMMS:
            for clear_csr in [True, False]:
