...
def collect_control_register_coverage(is_difuzzrtl: bool):
    if is_difuzzrtl:
        command = f"docker run -it -v {os.environ['CASCADE_DOCKER_MNT_DIR']}:/difuzzrtl {docker_img_name} bash -c 'cd /difuzzrtl && bash fuzz_difuzzrtl.sh'"
    else:
        command = f"docker run -it -v {os.environ['CASCADE_DOCKER_MNT_DIR']}:/difuzzrtl {docker_img_name} bash -c 'cd /difuzzrtl && bash fuzz_cascade.sh'"
