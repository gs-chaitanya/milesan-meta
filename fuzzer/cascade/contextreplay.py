...
def get_context_setter_max_size(fuzzerstate):
    ...
    if fuzzerstate.design_has_fpu:
        ...
        num_instrs_freg = 0
    # 4: 3 Instruction_t0s + 4-8 bytes (~1-2 instrs) to store the data + 4 bytes to potentially align.
...
def gen_context_setter(fuzzerstate, saved_context, next_jmp_addr: int):
    ...
    if fuzzerstate.design_has_fpu:
        ...
        curr_addr += 4 # NO_COMPRESSED
        # Load the fcsr value
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lwu" if fuzzerstate.is_design_64bit else "lw", 1, 1, 0, -1))
        curr_addr += 4 # NO_COMPRESSED
        # Write the value into fcsr
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.FCSR))
        curr_addr += 4 # NO_COMPRESSED
    ...
    if fuzzerstate.design_has_supervisor_mode:
        ...
        fuzzerstate.ctxsv_bbs[-1].append(None)
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lwu" if fuzzerstate.is_design_64bit else "lw", 1, 1, 0, -1))
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.SEPC))
        curr_addr += 12 # NO_COMPRESSED
    ...
    fuzzerstate.ctxsv_bbs[-1].append(None)
    fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lwu" if fuzzerstate.is_design_64bit else "lw", 1, 1, 0, -1))
    fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.MCAUSE))
    curr_addr += 12 # NO_COMPRESSED
    ...
    if fuzzerstate.design_has_supervisor_mode:
        ...
        fuzzerstate.ctxsv_bbs[-1].append(None)
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lwu" if fuzzerstate.is_design_64bit else "lw", 1, 1, 0, -1))
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.SCAUSE))
        curr_addr += 12 # NO_COMPRESSED
    ...
    fuzzerstate.ctxsv_bbs[-1].append(None)
    if fuzzerstate.is_design_64bit:
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"ld", 1, 1, 0, -1))
    else:
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lwu" if fuzzerstate.is_design_64bit else "lw", 1, 1, 0, -1))
    fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.MSCRATCH))
    curr_addr += 12 # NO_COMPRESSED
    ...
    if fuzzerstate.design_has_supervisor_mode:
        ...
        if fuzzerstate.is_design_64bit:
            ...
            fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lwu" if fuzzerstate.is_design_64bit else "lw", 1, 1, 0, -1))
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.SSCRATCH))
        curr_addr += 12 # NO_COMPRESSED
    ...
    if 'picorv32' not in fuzzerstate.design_name:
        ...
        fuzzerstate.ctxsv_bbs[-1].append(None)
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lwu" if fuzzerstate.is_design_64bit else "lw", 1, 1, 0, -1))
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.MTVEC))
        curr_addr += 12 # NO_COMPRESSED
    ...
    if fuzzerstate.design_has_supervisor_mode:
        ...
        fuzzerstate.ctxsv_bbs[-1].append(None)
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lwu" if fuzzerstate.is_design_64bit else "lw", 1, 1, 0, -1))
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.STVEC))
        curr_addr += 12 # NO_COMPRESSED
    ...
    if fuzzerstate.design_has_supervisor_mode:
        ...
        fuzzerstate.ctxsv_bbs[-1].append(None)
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lwu" if fuzzerstate.is_design_64bit else "lw", 1, 1, 0, -1))
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.MEDELEG))
        curr_addr += 12 # NO_COMPRESSED
    ...
    if fuzzerstate.is_design_64bit:
        fuzzerstate.ctxsv_bbs[-1].append(None)
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"ld", 1, 1, 0, -1))
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.MSTATUS))
        curr_addr += 12 # NO_COMPRESSED
    else:
        ...
        fuzzerstate.ctxsv_bbs[-1].append(None)
        fuzzerstate.ctxsv_bbs[-1].append(None)
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lw", 1, 1, 0, -1))
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lw", 2, 2, 0, -1))
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.MSTATUS))
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 2, CSR_IDS.MSTATUSH))
    ...
    if fuzzerstate.is_design_64bit:
        fuzzerstate.ctxsv_bbs[-1].append(None)
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"ld", 1, 1, 0, -1))
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.MINSTRET))
        curr_addr += 12 # NO_COMPRESSED
    else:
        # Register 1 contains the lsbs and register 2 the msbs.
        fuzzerstate.ctxsv_bbs[-1].append(None)
        ...
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lw", 2, 2, 0, -1))
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.MINSTRET))
        fuzzerstate.ctxsv_bbs[-1].append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 2, CSR_IDS.MINSTRETH))
        curr_addr += 24 # NO_COMPRESSED
    ...
    for mem_byte_id, (mem_byte_addr, mem_byte_val) in enumerate(saved_context.mem_bytes_dict.items()):
        ...
        fuzzerstate.ctxsv_bbs[-1].append(None) # will be replaced later on when addresses are written.
        curr_addr += 4 # NO_COMPRESSED
        # We load the byte addr into register 1
        fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lwu" if fuzzerstate.is_design_64bit else "lw", 1, 1, 0, -1, is_rd_nonpickable_ok=True))
        curr_addr += 4 # NO_COMPRESSED
        ...
        fuzzerstate.ctxsv_bbs[-1].append(inst)

        curr_addr += 4 # NO_COMPRESSED
        # Store the value into memory
        fuzzerstate.ctxsv_bbs[-1].append(IntStoreInstruction_t0(fuzzerstate,"sb", 1, 2, 0, -1)) # sb dest, val, offset
        curr_addr += 4 # NO_COMPRESSED
    ...
    if fuzzerstate.design_has_fpu:
        ...
        #     if fuzzerstate.design_has_fpud:
        #         fuzzerstate.ctxsv_bbs[-1].append(FloatLoadInstruction_t0(fuzzerstate,"fld", freg_id, 1, 8*freg_id, -1))
        #     else:
        #         fuzzerstate.ctxsv_bbs[-1].append(FloatLoadInstruction_t0(fuzzerstate,"flw", freg_id, 1, 4*freg_id, -1))
        #     curr_addr += 4 # NO_COMPRESSED
    ...
    for reg_id, reg_val in enumerate(saved_context.reg_vals):
        ...
        if fuzzerstate.is_design_64bit:
            fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"ld", reg_id, MAX_NUM_PICKABLE_REGS, 8*reg_id, -1, is_rd_nonpickable_ok=True))
            curr_addr += 4 # NO_COMPRESSED
        else:
            fuzzerstate.ctxsv_bbs[-1].append(IntLoadInstruction_t0(fuzzerstate,"lw", reg_id, MAX_NUM_PICKABLE_REGS, 4*reg_id, -1, is_rd_nonpickable_ok=True))
            curr_addr += 4 # NO_COMPRESSED

    # Reset the reg MAX_NUM_PICKABLE_REGS to 0
    fuzzerstate.ctxsv_bbs[-1].append(RegImmInstruction_t0(fuzzerstate,"addi", MAX_NUM_PICKABLE_REGS, 0, 0, is_rd_nonpickable_ok=True))
    curr_addr += 4 # NO_COMPRESSED
    ...
    if fuzzerstate.design_has_fpu:
        fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.FCSR])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.fcsr))
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,0xdeadbeef))
        curr_addr += 8 # NO_COMPRESSED

    if saved_context.privilege == PrivilegeStateEnum.MACHINE and (fuzzerstate.design_has_supervisor_mode or fuzzerstate.design_has_user_mode):
        fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.MEPC])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.mepc))
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,0xdeadbeef))
        curr_addr += 8 # NO_COMPRESSED

    if fuzzerstate.design_has_supervisor_mode:
        fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.SEPC])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.sepc))
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,0xdeadbeef))
        curr_addr += 8 # NO_COMPRESSED

    # mcause
    fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.MCAUSE])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
    fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.mcause))
    fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,0xdeadbeef))
    curr_addr += 8 # NO_COMPRESSED

    if fuzzerstate.design_has_supervisor_mode:
        fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.SCAUSE])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.scause))
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,0xdeadbeef))
        curr_addr += 8 # NO_COMPRESSED

    # mscratch
    fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.MSCRATCH])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
    fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.mscratch & 0xffffffff))
    fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.mscratch >> 32))
    curr_addr += 8 # NO_COMPRESSED

    if fuzzerstate.design_has_supervisor_mode:
        fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.SSCRATCH])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.sscratch & 0xffffffff))
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.sscratch >> 32))
        curr_addr += 8 # NO_COMPRESSED

    if 'picorv32' not in fuzzerstate.design_name:
        fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.MTVEC])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.mtvec))
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,0xdeadbeef))
        curr_addr += 8 # NO_COMPRESSED

    if fuzzerstate.design_has_supervisor_mode:
        fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.STVEC])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.stvec))
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,0xdeadbeef))
        curr_addr += 8 # NO_COMPRESSED

    if fuzzerstate.design_has_supervisor_mode:
        fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.MEDELEG])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.medeleg))
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,0xdeadbeef))
        curr_addr += 8 # NO_COMPRESSED
    ...
    fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.MSTATUS])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
    if not fuzzerstate.is_design_64bit:
        # Prepare register 2 to be written to the msb of mstatus
        fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.MSTATUS])+1] = RegImmInstruction_t0(fuzzerstate,"addi", 2, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr+4, is_rd_nonpickable_ok=True)
    fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.mstatus & 0xffffffff))
    fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,saved_context.mstatus >> 32))
    curr_addr += 8 # NO_COMPRESSED
    ...
    # Little endian. For a number written big endian `abcd`, the bytes should be written in memory as `dcba`. So we write the lsbs first.
    fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.MINSTRET])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
    if not fuzzerstate.is_design_64bit:
        # Prepare register 2 to be written to the msb of minstret
        fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_csr_loads[CSR_IDS.MINSTRET])+1] = RegImmInstruction_t0(fuzzerstate,"addi", 2, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr+4, is_rd_nonpickable_ok=True)

    fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,(saved_context.minstret - ((instr_end_addr - (minstret_base_addr - 4 - 4*int(fuzzerstate.is_design_64bit))) // 4)) & 0xffffffff, signed=True))

    if fuzzerstate.is_design_64bit:
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,(saved_context.minstret - ((instr_end_addr - minstret_base_addr - 4) // 4)) >> 32, signed=True))
    else:
        ...
        if (saved_context.minstret - ((instr_end_addr - (minstret_base_addr - 4 - 4*int(fuzzerstate.is_design_64bit))) // 4)) < 0: # If minstret is negative and will be increased before the start of the run
            fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,(((saved_context.minstret + (saved_context.minstreth << 32)) - ((instr_end_addr - minstret_base_addr - 4) // 4)) >> 32), signed=True))
        else:
            fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,(((saved_context.minstret + (saved_context.minstreth << 32)) - ((instr_end_addr - minstret_base_addr - 4) // 4)) >> 32), signed=True))
    curr_addr += 8 # NO_COMPRESSED
    ...
    for mem_byte_id, (mem_byte_addr, mem_byte_val) in enumerate(saved_context.mem_bytes_dict.items()):
        ...
        fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addrs_memaddrs[mem_byte_id])] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
        fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,mem_byte_addr + SPIKE_STARTADDR))
        curr_addr += 4 # NO_COMPRESSED
    ...
    if fuzzerstate.design_has_fpu:
        ...
        if fuzzerstate.design_has_fpud:
            # Align if needed
            if curr_addr % 8 != 0:
                fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,0))
                curr_addr += 4
            fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_freg_load_addi)] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
            for freg_id, freg_val in enumerate(saved_context.freg_vals):
                if DO_ASSERT:
                    assert freg_val >= 0
                    assert freg_val < 1 << 64
                fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,freg_val % (1 << 32)))
                fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,freg_val // (1 << 32)))
                curr_addr += 8
        else:
            if DO_ASSERT:
            ...
            fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_freg_load_addi)] = RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
            for freg_id, freg_val in enumerate(saved_context.freg_vals):
                fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,freg_val))
                curr_addr += 4 # NO_COMPRESSED
    ...
    if fuzzerstate.is_design_64bit:
        ...
        for reg_id, reg_val in enumerate(saved_context.reg_vals):
            ...
            fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,reg_val % (1 << 32), reg_val_t0 % (1 << 32))) # lower 32bit
            fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,reg_val // (1 << 32), reg_val_t0 // (1 << 32))) # upper 32bit
            curr_addr += 8
    else:
        fuzzerstate.ctxsv_bbs[-1][addr_to_id_in_ctxsv(addr_reg_load_addi)] = RegImmInstruction_t0(fuzzerstate,"addi", MAX_NUM_PICKABLE_REGS, MAX_NUM_PICKABLE_REGS, curr_addr-fuzzerstate.curr_ctxsv_bb_start_addr, is_rd_nonpickable_ok=True)
        for reg_id, reg_val in enumerate(saved_context.reg_vals):
            fuzzerstate.ctxsv_bbs[-1].append(RawDataWord_t0(fuzzerstate,reg_val,reg_val_t0))
            curr_addr += 4 # NO_COMPRESSED

    if DO_ASSERT:
        assert curr_addr == fuzzerstate.curr_ctxsv_bb_start_addr + len(fuzzerstate.ctxsv_bbs[-1])*4, f"curr_addr is `{hex(curr_addr)}`, fuzzerstate.curr_ctxsv_bb_start_addr + len(fuzzerstate.ctxsv_bbs[-1])*4 is `{hex(fuzzerstate.curr_ctxsv_bb_start_addr + len(fuzzerstate.ctxsv_bbs[-1])*4)}`" # NO_COMPRESSED
