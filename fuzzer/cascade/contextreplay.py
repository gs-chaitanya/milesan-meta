...
def gen_context_setter(fuzzerstate, saved_context, next_jmp_addr: int):
    ...
    if saved_context.privilege == PrivilegeStateEnum.SUPERVISOR or saved_context.privilege == PrivilegeStateEnum.USER:
        ...
        # fuzzerstate.ctxsv_bb.append(RegImmInstruction_t0(fuzzerstate,"addi", 1, MAX_NUM_PICKABLE_REGS, mepc_target-fuzzerstate.ctxsv_bb_base_addr, is_rd_nonpickable_ok=True)) # The reg `MAX_NUM_PICKABLE_REGS` contains the start address of the context sette, is_rd_nonpickable_ok=Truer
        # fuzzerstate.ctxsv_bb.append(CSRRegInstruction_t0(fuzzerstate,"csrrw", 0, 1, CSR_IDS.MEPC))
        # fuzzerstate.ctxsv_bb.append(PrivilegeDescentInstruction_t0(fuzzerstate,True)) # mret
    ...
    for mem_byte_id, (mem_byte_addr, mem_byte_val) in enumerate(saved_context.mem_bytes_dict.items()):
        ...
        if fuzzerstate.taint_en:
            ...
            fuzzerstate.memview.write_t0(inst.addr, inst.gen_bytecode_int_t0(False), 4)
        else:
    ...
    if fuzzerstate.design_has_fpu:
        ...
        #     if fuzzerstate.design_has_fpud:
        #         fuzzerstate.ctxsv_bb.append(FloatLoadInstruction_t0(fuzzerstate,"fld", freg_id, 1, 8*freg_id, -1))
        #     else:
        #         fuzzerstate.ctxsv_bb.append(FloatLoadInstruction_t0(fuzzerstate,"flw", freg_id, 1, 4*freg_id, -1))
        #     curr_addr += 4 # NO_COMPRESSED
    ...
    for id,instr in enumerate(fuzzerstate.ctxsv_bb):
        instr.addr = fuzzerstate.ctxsv_bb_base_addr + 4*id + SPIKE_STARTADDR
