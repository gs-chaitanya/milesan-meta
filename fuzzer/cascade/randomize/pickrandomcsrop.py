...
from cascade.toleratebugs import TOLERATE_KRONOS_MINSTRET, TOLERATE_VEXRISCV_MINSTRET, TOLERATE_PICORV32_MISSINGMANDATORYCSRS, TOLERATE_PICORV32_WRITEHPM, TOLERATE_CVA6_MHPMCOUNTER, TOLERATE_CVA6_MHPMEVENT31
...
def gen_random_csr_op(fuzzerstate):
    ...
    if fuzzerstate.privilegestate.privstate == PrivilegeStateEnum.MACHINE:
        if fuzzerstate.is_design_64bit:
            ...
            if not fuzzerstate.design_has_supervisor_mode:
                while target_csr in (MachineCSROpCandidates32.SCAUSE, MachineCSROpCandidates32.SSCRATCH):
                    target_csr = random.choice(list(MachineCSROpCandidates32))
            if target_csr == MachineCSROpCandidates32.SCAUSE:
                # According to the spec, the SCAUSE CSR must be able to hold bits 0 to 4. mret is not required to.
                if fuzzerstate.design_name == "vexriscv": # vexriscv complies with the privileged spec v1.10, which does not require scause to hold the 5th bit. Similarly, kronos implements privileged spec v1.11
                ...
                if fuzzerstate.design_name == "kronos" and not TOLERATE_KRONOS_MINSTRET or fuzzerstate.design_name == "vexriscv" and not TOLERATE_VEXRISCV_MINSTRET:
