...
def gen_new_test_instance(design_name: str, randseed: int):
    return random.randrange(1 << 14, 1 << LOG2_MEMSIZE_UPPERBOUND), design_name, randseed, random.randrange(20, NUM_MAX_BBS_UPPERBOUND)
...
def gen_fuzzerstate_elf_expectedvals(memsize: int, design_name: str, check_pc_spike_again: bool = False, randseed: int = 0, nmax_bbs: int = None):
    ...
    fuzzerstate = FuzzerState(get_design_boot_addr(design_name), design_name, memsize, randseed, nmax_bbs)
