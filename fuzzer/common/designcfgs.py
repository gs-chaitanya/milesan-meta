...
# @param design_name: must be one of the keys of the design_repos.json dict.
# @return the march flags for the design, for example `-march=rv64gc -mabi=lp64`.
def get_design_march_ccflags_nocompressed(design_name) -> int:
    device_config = get_design_cfg(design_name)
    return device_config["marchflags"].replace('c', '')
...
# @param design_name: must be one of the keys of the design_repos.json dict.
# @return the trap signal address of the design: the address to which to write to signal that the CPU trapped.
def get_design_trap_sig_addr(design_name) -> int:
    device_config = get_design_cfg(design_name)
    return int(device_config["trapsigaddr"], base=0)
