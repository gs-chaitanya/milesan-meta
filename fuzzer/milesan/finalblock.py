...
from rv.asmutil import li_into_reg, li_top_down_ops
...
def finalblock(fuzzerstate, design_name: str):
    ...
    if DO_ASSERT:
        assert regdump_addr < 0x100000000, f"For the destination address `{hex(regdump_addr)}`, we will need to manage sign extension, which is not yet implemented here."
        assert stopsig_addr < 0x100000000, f"For the destination address `{hex(stopsig_addr)}`, we will need to manage sign extension, which is not yet implemented here."
    ...
    # NOTE: Regdump section commented out for Chipyard HTIF integration.
    # The regdump address construction and register dump loops are not needed
    # when targeting Chipyard verilator with HTIF/fesvr termination.
    #
    ...
    if fuzzerstate.effective_curr_layout == -1 and  not (fuzzerstate.privilegestate.privstate == PrivilegeStateEnum.MACHINE and mprv_bit and (fuzzerstate.privilegestate.curr_mstatus_mpp != PrivilegeStateEnum.MACHINE and fuzzerstate.real_curr_layout != -1)):
        ...
        # Build stopsig address top-down using slli+addi (avoids sign-extension on RV64).
        # Each addi uses an 11-bit unsigned chunk (0-2047), always positive in signed
        # 12-bit immediate. slli shifts in clean zeros — no sign corruption.
        addr_ops = li_top_down_ops(stopsig_addr)
        for i, (op, val) in enumerate(addr_ops):
            if op == 'addi':
                rs1 = 0 if i == 0 else MPP_BOTH_ENDIS_REGISTER_ID
                ret.append(RegImmInstruction(fuzzerstate, "addi", MPP_BOTH_ENDIS_REGISTER_ID, rs1, val, is_rd_nonpickable_ok=True))
            elif op == 'slli':
                ret.append(RegImmInstruction(fuzzerstate, "slli", MPP_BOTH_ENDIS_REGISTER_ID, MPP_BOTH_ENDIS_REGISTER_ID, val, is_rd_nonpickable_ok=True))
    else:
    ...
    # Store value 1 to tohost (HTIF PASS signal: (exit_code << 1) | 1, exit_code=0 → value=1)
    ret += [
        RegImmInstruction(fuzzerstate, "addi", RPROD_MASK_REGISTER_ID, 0, 1, is_rd_nonpickable_ok=True),
        IntStoreInstruction(fuzzerstate, "sd" if fuzzerstate.is_design_64bit else "sw", MPP_BOTH_ENDIS_REGISTER_ID, RPROD_MASK_REGISTER_ID, 0, -1),
        SpecialInstruction(fuzzerstate, "fence"),
    ]
