...
def plot_rfuzz(active_rfuzz_coverage_path_per_design: dict, passive_rfuzz_coverage_path_per_design: dict):
    ...
    # Load the coverage data and durations for disco on rfuzz coverage
    coverage_durations_dicts_disco = {}
    for design_name in design_names:
        ...
        if not os.path.exists(json_path):
            ...
            with open(json_path, 'r') as f:
                coverage_durations_dicts_disco[design_name] = json.load(f)
    ...
    coverage_dict_disco = {design_name: 100*np.array(coverage_durations_dicts_disco[design_name]['coverage_sequence'])/NUM_COVERAGE_POINTS[design_name] for design_name in design_names}
    durations_seconds_dict_disco = {design_name: np.array(coverage_durations_dicts_disco[design_name]['durations']) for design_name in design_names}

    # Cumulative sum of durations
    durations_seconds_dict_cumsum_disco = {design_name: [sum(durations_seconds_dict_disco[design_name][0:id_in_tuple]) for id_in_tuple in range(len(durations_seconds_dict_disco[design_name]))] for design_name in durations_seconds_dict.keys()}
    ...
    for i, ax in enumerate(axs):
        ...
        X_disco = durations_seconds_dict_cumsum_disco[design_name]
        Y_disco = np.array(coverage_dict_disco[design_name])

        # Truncate after X_SECONDS_AFTER_RFUZZ_END more
        if X_SECONDS_AFTER_RFUZZ_END is not None:
            for j, x in enumerate(X_disco):
        ...
        # Extend X and Y to the max of X_disco
        X = np.append(X, X_disco[-1])
        Y = np.append(Y, Y[-1])

        ax.plot(X_disco, Y_disco, zorder=3, color='k', label='Cascade')
