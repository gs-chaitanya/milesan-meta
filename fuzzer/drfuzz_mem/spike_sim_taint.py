    ...
    for (pc0,rd0),(pc1,rd1) in zip(pc_reg_pairs_0.items(),pc_reg_pairs_1.items()):
        ...
        for (reg0_id, reg0_val),(reg1_id,reg1_val) in zip(rd0.items(),rd1.items()):
            ...
            if reg0_val^reg1_val:
                print(f"(spike) Taint vector at pc {hex(SPIKE_STARTADDR+pc0)} for reg(s) {set([ABI_INAMES[reg_id] for reg_id in [reg0_id,reg1_id]])}: {hex(reg0_val^reg1_val)}")
            pc_reg_taint_pairs[pc0][reg0_id] = reg0_val^reg1_val if reg0_id else 0# zero wont get tainted
