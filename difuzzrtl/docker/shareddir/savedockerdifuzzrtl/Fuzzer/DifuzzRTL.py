...
from CascadeFuzzer import RunCascade
from Minimizer import Minimize

from RTLSim.src.adapters.tilelink.adapter import IS_CASCADE
...
parser.add_option('cascade', IS_CASCADE, 'Whether to run cascade instead of difuzzrtl')
...
print("parser.arg_map['cascade'][0]", parser.arg_map['cascade'][0])
print("parser.arg_map['toplevel'][0]", parser.arg_map['toplevel'][0])
exit(0)
...
if not multicore:
    if minimize:
        ...
        if parser.arg_map['cascade'][0]:
            factory = TestFactory(RunCascade)
        else:
