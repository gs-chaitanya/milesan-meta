...
from RTLSim.src.adapters.tilelink.utils import *
...
class tlAdapter():
    ...
    def get_d_messages(self, message, memory, burst_len, addr_aligned, **kwargs):
        ...
        for i in range(burst_len):
            ...
            if get_addr not in memory.keys():
                ...
                # print('Returning NOP data at addr {}'.format(hex(get_addr)))
            # else:
            #     print('Returning mem data at addr {}: {}'.format(hex(get_addr), hex(memory[get_addr])))
    ...
    def drive_input(self, memory):
        assert memory.__class__.__name__ == 'dict' or IS_CASCADE, \
        ...
        if not IS_CASCADE:
            ...
            block_perm = defaultdict(lambda: TIP)
        ...
        while self.drive:
            if a_ports.fire():
                ...
                if opcode == GET:
                    ...
                    # print('TL -- GET addr {:#x}'.format(addr), flush=True)
                ...
                if opcode == PUT_FULL_DATA:
                    # print('TL -- PUT_FULL_DATA {:#x}'.format(addr), flush=True)

                    if addr == 0x60000000 and IS_CASCADE:
                        print('[Cascade] Found stop request', flush=True)
                        self.prompt_stop = True
                ...
                if opcode == PUT_PARTIAL_DATA:
                    # print('TL -- PUT_PARTIAL_DATA {:#x}'.format(addr), flush=True)
                ...
                if opcode == ARITHMETIC_DATA and \
                   self.protocol >= TL_UH:
                    # print('TL -- ARITHMETIC_DATA', flush=True)
                ...
                if opcode == LOGICAL_DATA and \
                   self.protocol >= TL_UH:
                    # print('TL -- LOGICAL_DATA', flush=True)
                ...
                if opcode == ACQUIRE_BLOCK and \
                   self.protocol == TL_C:
                    ...
                    # print('TL -- ACQUIRE_BLOCK', flush=True)
                ...
                if opcode == ACQUIRE_PERM and \
                   self.protocol == TL_C:
                    # print('TL -- ACQUIRE_PERM', flush=True)
    ...
    def host_interface(self, block_perm, b_srcs, b_callback):
        ...
        while self.drive:
            if (self.probe & self.probe_en) and self.probe_addr not in self.ongoing_tlc.values():
                ...
                if block_perm[block_addr] != TIP or IS_CASCADE:
    ...
    # Returns True iff the design requests stopping
    def poll_stop_disco(self):
        return self.prompt_stop
