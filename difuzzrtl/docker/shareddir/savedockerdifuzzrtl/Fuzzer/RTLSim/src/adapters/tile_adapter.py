...
from RTLSim.src.adapters.tilelink.adapter import tlAdapter, IS_CASCADE
...
class tileAdapter():
    ...
    def poll_stop_disco(self):
        return self.tl_adapter.poll_stop_disco()

    def check_assert(self):
        return self.dut.metaAssert.value

    def start(self, memory, ints):
        if memory.__class__.__name__ != 'dict' and not IS_CASCADE:
