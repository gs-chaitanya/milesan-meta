...
from RTLSim.src.reader.tile_reader import tileSrcReader
from RTLSim.src.adapters.tile_adapter import tileAdapter
from RTLSim.src.adapters.tilelink.definitions import *
...
class rvRTLhost():
    ...
    def run_test(self, rtl_input: rtlInput, assert_intr: bool):
        ...
        for addr in memory.keys():
            if addr not in bootrom_addrs and addr < DRAM_BASE:
                # print(f"Bootrom addrs: {' '.join(list(map(hex, bootrom_addrs)))}")
                # print (f"Bad addr: {hex(addr)}, dram_base: {DRAM_BASE}", flush=True)
                mem_check = False
                # break

        if not mem_check:
            return (ILL_MEM, self.get_covsum())
        # print (f"Only good addresses!", flush=True)
    ...
    @coroutine
    def run_test_disco(self, rtl_input_hex: str):
        print('[RTLHost-Cascade] Start RTL simulation', flush=True)

        fd = open(rtl_input_hex, 'r')
        lines = fd.readlines()
        # for line_id, line in enumerate(lines):
        #     print(f"Read line at addr {hex(DRAM_BASE + line_id*8): {line}}", line, flush=True, end='')
        fd.close()

        max_cycles = 10000000

        tohost_addr = 0x60000000
        ...
        memory = defaultdict(lambda: TIP)
        for line_id, line in enumerate(lines):
            memory[DRAM_BASE + line_id*8] = int(line, 16)

        # Add the bootrom
        (_, bootrom_memory) = self.set_bootrom()
        for addr in bootrom_memory.keys():
            memory[addr] = bootrom_memory[addr]

        self.adapter.start(memory, {})
        for i in range(max_cycles):
            yield clkedge

            if self.adapter.poll_stop_disco():
                break
        ...
        if i == max_cycles - 1:
            print('[RTLHost] Timeout, max_cycle={}'.format(max_cycles), flush=True)

        print('[RTLHost-Cascade] Stop RTL simulation', flush=True)
